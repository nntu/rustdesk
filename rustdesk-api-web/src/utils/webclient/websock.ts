...
export default class Websock {
  ...
  sendMessage(json: message.DeepPartial<message.Message>) {
    const data = message.Message.encode(message.Message.fromPartial(json)).finish();
  ...
  sendRendezvous(data: rendezvous.DeepPartial<rendezvous.RendezvousMessage>) {
    this._websocket.send(
      rendezvous.RendezvousMessage.encode(rendezvous.RendezvousMessage.fromPartial(data)).finish(),
    );
  }
  ...
  async open(timeout = 12000): Promise<Websock> {
    return new Promise((resolve, reject) => {
      ...
      this._websocket.onerror = (e: any) => {
        if (!this._status) {
          reject(
            'Failed to connect to ' + (this._isRendezvous ? 'rendezvous' : 'relay') + ' server',
          );
          return;
        }
        this._status = e;
  ...
  async next(timeout = 12000): Promise<rendezvous.RendezvousMessage | message.Message> {
    const func = (
