import {
  batchDelete as admin_batchDelete,
  list as admin_list,
  remove as admin_remove,
} from '@/api/login_log';
import {
  batchDelete as my_batchDelete,
  list as my_list,
  remove as my_remove,
} from '@/api/my/login_log';
...
const apis = {
  admin: {
    batchDelete: admin_batchDelete,
    list: admin_list,
    remove: admin_remove,
    fetchPeers: admin_fetchPeers,
  },
...
export function useRepositories(api_type = 'my') {
  ...
  const getList = async () => {
    ...
    if (res) {
      ...
      const uuids = res.data.list
        .filter((item) => item.uuid && item.client === 'client' && !item.device_id)
        .map((item) => item.uuid);
      if (uuids.length > 0) {
